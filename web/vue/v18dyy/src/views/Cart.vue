...
export default {
  ...
  computed: {
    checkAllFlag() {//判断是否是全选，这样可以方便地实现全选按钮时勾上还是取消
  ...
  methods: {
    ...
    delCartConfirm(item) {
      this.delItem = item; //点击删除按钮时，保存当前要删除的对象(以便知道是删除哪个，因为所有的删除共用一个模态框)
        ...
        .then(response => {
          ...
          if (res.status == "0") {
            if (flag == "add" || flag == "minu") {
              //原来的代码这里有个bug，点击一次勾选或取消都会造成cartCount-1
    ...
    toggleCheckAll() {
      var flag = !this.checkAllFlag;//标识是全选还是取消全选
      this.cartList.forEach(item => {
