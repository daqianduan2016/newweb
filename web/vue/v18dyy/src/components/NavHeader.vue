...
<script>
    ...
    import { mapState } from 'vuex'//vuex的辅助函数
    export default{
        data(){
            return{
              userName:'admin',
        ...
        computed:{
          ...mapState(['nickName','cartCount'])//vuex里面的数据放在computed里面，实时计算，虽然这个项目意义不大，但是如果项目里的这个数据在多个地方显示，就必须这么做
        },
        /*nickName(){
        ...
        methods:{
            checkLogin(){
                axios.get("/users/checkLogin").then((response)=>{
                    ...
                    var path = this.$route.pathname;//从路由中取数据
                    if(res.status=="0"){
//                      this.nickName = res.result;
                      this.$store.commit("updateUserInfo",res.result);//提交mutation用commit，提交action用dispatch
